...
function ScoreBar({ label, value, negative = false }: BarProps) {
  ...
  return (
    <div className="flex items-center gap-2 text-xs">
      <span className="w-44 shrink-0 truncate text-zinc-400" title={label}>
        {label}
      </span>
...
function Category({ title, avg, children }: CategoryProps) {
  const p = pct(avg);
  const color = p >= 75 ? 'text-emerald-400' : p >= 50 ? 'text-amber-400' : 'text-red-400';
  return (
...
export function MicroScorePanel({ scores, label }: MicroScorePanelProps) {
  ...
  return (
    <div className="animate-fade-in mt-4 space-y-4 rounded-xl border border-zinc-700 bg-zinc-900/80 p-5">
      <div className="flex items-center justify-between border-b border-zinc-800 pb-3">
        <div>
          <p className="text-sm font-semibold text-zinc-100">Micro-Segmentation Analysis</p>
          <p className="text-xs text-zinc-500">{label}</p>
        </div>
        <div className="text-right">
          <p className="text-2xl font-bold text-amber-500">{pct(fo.finalClipScore)}</p>
          <p className="text-xs text-zinc-500">clip score</p>
        </div>
      </div>

      {/* Final output composite scores */}
      <div className="rounded-lg border border-zinc-800 bg-zinc-950/40 p-3">
        <p className="mb-2 text-xs font-semibold uppercase tracking-wider text-zinc-400">Composite Outputs</p>
        ...
        <Category title="Scene Understanding" avg={suAvg}>
          <div className="space-y-0.5 pb-1">
            <p className="text-xs text-zinc-500">
              <span className="text-zinc-400">Scene: </span>{su.sceneCategory}
            </p>
            <p className="text-xs text-zinc-500">
              <span className="text-zinc-400">Environment: </span>{su.environmentType}
            </p>
            <p className="text-xs text-zinc-500">
              <span className="text-zinc-400">Setting: </span>{su.indoorOutdoor}
            </p>
            <p className="text-xs text-zinc-500">
              <span className="text-zinc-400">Activity: </span>{su.activityType}
            </p>
          </div>
