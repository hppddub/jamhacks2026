...
function buildGlobalStyles(a: VideoAnalysis): { positive: string[]; negative: string[] } {
  ...
  const positive = dedupe([
    ...
    `${a.pace} pace`,
    ...a.instrumentSuggestions.slice(0, 3),
...
function buildSection(
  ...
  // Fold in the segment's sound-only musical description for richer, textured sections.
  const desc = clean(seg.musicalDescription);
  return {
    section_name: name,
    positive_local_styles: dedupe([
      seg.mood,
      `${seg.energyLevel} energy`,
      ...(desc ? [desc] : []),
      ...(posHint ? [posHint] : []),
    ]),
